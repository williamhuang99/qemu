        ...
        target_long imm)
{
    ...
    if (gen_mem_trace()) {
        TCGv t2 = tcg_const_i32(ctx->base.pc_next);
        switch (memop) {
        case MO_SB:
        ...
        target_long imm)
{
    ...
    if (gen_mem_trace()) {
        TCGv t1 = tcg_const_i32(ctx->base.pc_next);
        switch (memop) {
        case MO_SB:
        case MO_UB:
...
static void riscv_tr_tb_stop(DisasContextBase *dcbase, CPUState *cpu)
{
    ...
    if (gen_tb_trace()) {
        if (cpu->singlestep_enabled) {
            /* exit on singlestep. */
            csky_trace_tb_exit(0x1, ctx->base.pc_next - ctx->base.pc_first);
        } else if (dcbase->is_jmp == DISAS_NEXT) {
            /* exit on special insns. */
            csky_trace_tb_exit(0x2, ctx->base.pc_next - ctx->base.pc_first);
        } else if (dcbase->is_jmp == DISAS_TOO_MANY) {
            /* exit on too many insns. */
            csky_trace_tb_exit(0x3, ctx->base.pc_next - ctx->base.pc_first);
        }
    }
