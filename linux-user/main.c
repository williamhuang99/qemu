...
static void handle_arg_version(const char *arg)
{
    ...
    char osver[] = "[linux 4.x]";
#else
    char osver[] = "[linux 3.x]";
#endif
    printf("qemu-" TARGET_NAME " for %s version " QEMU_FULL_VERSION
           "\n" QEMU_COPYRIGHT "\n", osver);
    exit(EXIT_SUCCESS);
}
...
static const struct qemu_argument arg_table[] = {
    ...
    {"csky-trace", "CSKY_TRACE",       true,  handle_arg_csky_trace,
     "",           "[port=<port>][,tb_trace=<on|off>][,mem_trace=<on|off>][,auto_trace=<on|off>]"},
...
int main(int argc, char **argv, char **envp)
{
...
#if ((defined TARGET_CSKY) || (defined TARGET_RISCV32))
    ...
    if (trace_opts) {
        ...
        if (str != NULL) {
            port = atoi(str);
            if (port) {
                traceserver_start(port, (gdbstub_port != 0));
            } else {
